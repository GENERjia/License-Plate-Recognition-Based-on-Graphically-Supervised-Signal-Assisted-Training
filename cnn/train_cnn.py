...
def get_parser():
    ...
    parser.add_argument('--lpr_max_len', default=2, help='license plate number max length.')
    ...
    # parser.add_argument('--pretrained_model', default='./weights/pretrain.pth', help='pretrained base model')
...
def train():
    ...
    model = build_alex(phase=args.phase_train, class_num=len(CHARS))
    ...
    # optimizer = optim.RMSprop(model.parameters(), lr=args.learning_rate, alpha = 0.9, eps=1e-08,
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if iteration != 0 and iteration % args.save_interval == 0:
            torch.save(model.state_dict(), args.save_folder + 'LPRNet_' + '_iteration_' + repr(iteration) + '.pth')
        ...
        end_time = time.time()
        if iteration % 20 == 0:
            metrics['CTCLoss'].append(loss.item())
    ...
    torch.save(model.state_dict(), args.save_folder + 'Final_LPRNet_model.pth')
...
if __name__ == "__main__":
    ...
    with open('weights/logs.json', 'w') as f:
        f.write(metrics)
